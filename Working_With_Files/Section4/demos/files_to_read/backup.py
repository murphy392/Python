...
from ftbsync.targets import FsTarget
...
class Backup(Base):
        ...
        if not Base.hasended() and localfolder != '' and remotefolder != '':
            if cls.__numfiles(localfolder) > 0:
                ...
                try:
                    ...
                    output = '>> ' + str(count) + ' files FTPed successsfully to: ' + remotefolder
    ...
    def __openzip(cls, chkmd5, target_zip):
        result = UploadSynchronizerif not Base.hasended():
        if not Base.hadended():
            if chkmd5:
        ...
        if not Base.hasended():
            if chkmd5:
                ...
                if not cls.__isomitted(fname, sourcef):
                    finzip = os.path.join(subdir, fname)
                    count += 1
                    cls.progrss(color, numfile, total, fname)
                    zipf.write(finzip)
                else:
                    msg = fname + ' -> skipped (omitted)'
                    cls.progress(color, numfile, total, fname)
    ...
    def __closezip2(cls, zipf, count, target_zip):
        try:
            ...
            print colored(msg + taregt_zip, 'green')
    ...
    def __dozip(cls, source_folder, target_zip, color, usedb):
        ...
        zipf = None
        if not Base.hasended() and total > 0 and sourcefolder != '' and target_zip != '':
                ...
                for fname in files:
                    ...
                    if not cls.__isomitted(fname, subdir):
                        if force or cls.__md5db(fname, source_folder, subdir, usedb):
                            ...
                            cls.progress(color, item, total, 'Inspecting -> ' + fname + ' (for omitting)')
                        item += 1
    ...
    def __docopy(cls, source_folder, target_folder, color, force, usedb):
        ...
        if not Base.hasended() and source_folder != '' and target_folder != '':
            if total > 0:
                ...
                Base.log(msg, false)
                try:
                    source = (source_folder, target_folder, force, 0 , count)
                    count = cls.__coredocopy(source, color, total, usedb)
                except IOError, ioerror:
                    count -= 1
        ...
        if not Base.hasended():
            ...
            for ssline in sss:
                if Base.hasended():
                    ...
                    numparts = len(sitem)
                    if numparts > 0:
                        server = {'ftp': sitem[0], 'user': sitem[1], 'password': sitem[2]}
    ...
    def __isomitted(cls, filename, source_folder):
        result = False
        ff = ospath.join(source_folder, filename)
        ...
        for fitem in items:
            if Base.hasended():
                ...
                if prts[5].strip() == 'z':
                    ...
                    count += cls.__docopy(sourcef, prts[1], cls.validcolor(parts[4], True, usedb))
        return count
